...
WEIGHTS_NAME = "facenality-weights-neutral-05-02-2019.h5"
...
if __name__ == "__main__":
    ...
    img = read_img("../dataset/predict-cropped/neutral/211.jpg")
    img = np.expand_dims(img, axis = 0)
    a = keract.get_activations(model, img[0:1])
    keract.display_activations(a)
    
    # WORKS LIKE A CHARM - WANTED RESULT
    img_array = np.load('conv.npy')
    ...
    # PRINT CONV IMAGE
    img_a = a["conv2d_35/Relu:0"]
    img_a = img_a[0, :, :, 0]
    plt.imshow(img_a, interpolation='nearest')
    plt.show()
    
    # HOW TO SAVE/ DISPLAY AN IMAGE
    """
